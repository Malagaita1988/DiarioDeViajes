// entries.js
// Asegúrate de que config.js se cargue antes que este archivo.
// Si usas módulos ES6, puedes importarlo así:
// import { API_BASE_URL } from './config.js';
// Si no usas módulos, asegúrate de que API_BASE_URL esté definida globalmente en config.js.
...
function renderEntries(entries) {
  ...
  // Recuperar favoritos almacenados en localStorage
  const favoritos = new Set(JSON.parse(localStorage.getItem("favoritos") || "[]"));

  // Se muestran las entradas hasta "visibleEntries"
...
function filterEntries(searchText, category = 'all') {
  ...
  return allEntries.filter(entry => {
    ...
    const matchesCategory = category === 'all' || (entry.category && entry.category.toLowerCase() === category.toLowerCase());
